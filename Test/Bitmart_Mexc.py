...
def calculate_profit(symbol):
   ...
   # Рассчитываем среднюю цену покупки и продажи, объем покупки и продажи, количество ордеров для покупки и продажи полученного объема и процент прибыли
    total_purchase_volume = 0
    total_purchase_price = 0
    total_sale_volume = 0
    total_sale_price = 0
    purchase_orders = min(len(ask_bitmart), 10)
    sale_orders = min(len(bid_mexc), 10)
    for ask in ask_bitmart[:purchase_orders]:
        ask_price = float(ask['price'])
        ask_volume = float(ask['amount'])
        for bid in bid_mexc[:sale_orders]:
            bid_price = float(bid[0])
            bid_volume = float(bid[1])
            if ask_price < bid_price:
                volume = min(ask_volume, bid_volume)
                total_purchase_volume += volume
                total_purchase_price += volume * ask_price
                total_sale_volume += volume
                total_sale_price += volume * bid_price
                ask_volume -= volume
                if ask_volume == 0:
                    break

    average_purchase_price = total_purchase_price / \
        total_purchase_volume if total_purchase_volume > 0 else 0
    average_sale_price = total_sale_price / \
        total_sale_volume if total_sale_volume > 0 else 0
    profit_percentage = (total_sale_price - total_purchase_price) / \
        total_purchase_price * 100 if total_purchase_price > 0 else 0

    return {
        'average_purchase_price': average_purchase_price,
        'average_sale_price': average_sale_price,
        'purchase_volume': total_purchase_volume,
        'sale_volume': total_sale_volume,
...
results = calculate_profit(symbol)
purchase_volume_usdt = results['average_purchase_price'] * \
    results['purchase_volume']
sale_volume_usdt = results['average_sale_price'] * results['sale_volume']
print(f"Объем покупки (USDT): {purchase_volume_usdt}")
print(f"Объем продажи (USDT): {sale_volume_usdt}")

print(f"Средняя цена покупки: {results['average_purchase_price']}")
print(f"Средняя цена продажи: {results['average_sale_price']}")

print(f"Количество ордеров для покупки: {results['purchase_orders']}")
print(f"Количество ордеров для продажи: {results['sale_orders']}")
print(f"% прибыли: {results['profit_percentage']}")
